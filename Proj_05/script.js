...
//Functions to filter the users and only shows millionaires
...
//Function to calculate the total networth of all the users
function calculateTotalNetWorth(){
    const totalWorth = data.reduce(
        (acc, item) => (acc += item.worth), 0
            ...
            totalNetWorthElement.innerHTML = `<h3>Total NetWorth: <strong>${formatCurrency(totalWorth)}</strong></h3>`;
...
// Function Update The UI with DOM
...
//Function to Format a number as a currrency
